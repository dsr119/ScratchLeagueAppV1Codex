import {actualSeasonAudit} from './actual-season.js';
import {MODEL_VERSION,currentWeek,profile,standings,matchPoints} from './model.js';
import {calculateStats,calculateAllPlay,analyticsChecks,STATS_FORMULAS,STATS_VERSION,defaultMinimum} from './stats.js';
...
export function createDebugReport(state,run,context={}){
 ...
 const actualAudit=actualSeasonAudit(state);checks.push(...actualAudit.checks);
 const safeStats=(settings={})=>{try{return calculateStats(state,settings);}catch(e){return {source:settings.source||'current',minimum:settings.minimum??defaultMinimum(state),bowlers:[],qualified:[],error:e.message};}};
 const stats=safeStats(),allPlay=calculateAllPlay(state);
 checks.push(...analyticsChecks(state,stats,allPlay));
 const settings=context.analyticsSettings||{},selectedStats=safeStats({source:settings.source||'current',category:settings.category||'all',minimum:settings.minimum??defaultMinimum(state,settings.source||'current')});
 checks.push(...analyticsChecks(state,selectedStats,allPlay).filter(c=>c.name.startsWith('Stats minimum')||c.name.startsWith('Stats rating')).map(c=>({...c,name:'Selected '+c.name})));
 const analytics={version:STATS_VERSION,formulas:STATS_FORMULAS,stats,selectedStats,allPlay};
 ...
 const workspace=structuredClone({version:state.version,season:state.season,teams:state.teams,bowlers:state.bowlers,schedule:state.schedule,results:state.results,adjustments:state.adjustments,thirdWinners:state.thirdWinners,imports:state.imports,dataReview:state.dataReview,categoryAssignment:state.categoryAssignment});
 return {reportVersion:3,statisticSources:{actualSeason:'state.results[].matches[].players[] keyed by bowlerId',historical:'bowler.history for the selected prior season',model:'profile(): historical + entering + current model estimate',projected:'simulation outputs, never used as actual results'},workspacePersistence:context.persistence?{source:context.persistence.source,readAt:context.persistence.readAt,revision:context.persistence.revision,weeks:context.persistence.weeks,week3:context.persistence.week3,verifiedSave:context.persistence.verifiedSave===true}:{source:'Not verified by a Supabase read in this session'},actualSeasonPlayerStats:actualAudit.actualSeasonPlayerStats,week3Audit:actualAudit.week3,categoryAudit:stats.categories??null,analytics,generatedAt:new Date().toISOString(),modelVersion:MODEL_VERSION,runtime,summary:{completedWeeks:currentWeek(state),historicalGames:ratings.reduce((a,b)=>a+b.priorGames,0),currentActualGames:ratings.reduce((a,b)=>a+b.currentGames,0),failedChecks:checks.filter(c=>c.status==='fail').length,hasSimulation:!!run},checks,ratings,standings:standings(state),workspace,selectedSimulation:run?structuredClone(run):null,runHistory:state.runs.map(r=>({id:r.id,createdAt:r.createdAt,modelVersion:r.modelVersion,completedWeeks:r.completedWeeks,iterations:r.iterations,seed:r.seed,adjustments:r.adjustments,teams:r.teams})),limitations:['Checks identify structural and arithmetic problems; they do not prove forecast accuracy.','No live Supabase server inspection is performed by this download.','Weekly odds cover regular-season matchups. Historical odds are retained runs, not reconstructed forecasts.']};
}

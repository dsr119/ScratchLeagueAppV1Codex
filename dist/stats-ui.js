...
function renderStatsView(state,settings){
 ...
 if(tab==='overview'){
  ...
  if(source==='current')body+=panel('Series leaders',`<div class="panel-body">${esc(STATS_FORMULAS.seriesLeaders)}</div>`+table('series-leaders',[...base,C('seriesCount','3-game sessions'),C('pairSeriesLeads','Highest series on pair'),C('teamSeriesLeads','Highest series on team')],rows,settings,'pairSeriesLeads'));
 ...
 (state.categoryAssignment?.method==='entering-average-thirds-v1'?`<div class="notice">Categories were assigned from entering averages: highest ${state.categoryAssignment.groupSize} regulars = A, next ${state.categoryAssignment.groupSize} = B, lowest ${state.categoryAssignment.groupSize} = C. Ties use alphabetical name order. Save league to keep newly assigned categories.</div>`:'')+(unassigned?`<div class="notice">${unassigned} regular bowler(s) have no verified A/B/C category. Best A/B/C rankings are disabled until every regular has a valid category. Categories use three equal groups of regular bowlers ranked by entering average.</div>`:'')+
...
export function statsDetail(state,id,settings){
 ...
 const fields=[['Current-season average',current.average],['Selected-period average',b.average],['Entering average',b.entering],['Improvement',b.improvement],['Games',b.games],['Total pins',b.totalPins],['High game',b.highGame],['Low game',b.lowGame],['High 3-game series',b.highSeries],['Highest series on pair · times',b.pairSeriesLeads],['Highest series on team · times',b.teamSeriesLeads],['Low 3-game series',b.lowSeries],['Average 3-game series',b.averageSeries],['Standard deviation',b.sd],['Median game',b.median],['Last 3-week average',b.last3],['Last 3-week games',b.last3Games],['Last 5-week average',b.last5],['Last 5-week games',b.last5Games],['Best Bowler Rating',b.rating??null]];
